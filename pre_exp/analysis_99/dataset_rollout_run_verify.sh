...
MODEL_PREFIX="../eval_results/rollout_analysis/DS7B_8k_redundancy_self_1_5e-4_1_correct_DAPO/global_step_100"
BASE_DIR="../../eval/redundancy/DS7B_8k_redundancy_self_1_5e-4_1_correct_DAPO/global_step_100"
MIN_SCORE=2
...
echo
echo "▶ 运行命令:"
echo "python dataset_verify_analysis.py \\"
...
# 可选参数：是否只统计正确样本
if [ "$TOKENS_CORRECT_ONLY" = "true" ]; then
  PYTHON_ARGS+=("--tokens_correct_only")
  echo "✅ 启用: 只统计正确样本的token数"
fi
...
# 执行命令 - 原来的正确方式
python dataset_rollout_verify_analysis.py \
  --inputs "${INPUT_FILES[@]}" \
  --datasets "${DATASET_NAMES[@]}" \
  --output_dir "$OUTPUT_DIR" \
  --extraction "$EXTRACTION" \
...
for DATASET in "${DATASET_NAMES[@]}"; do
  ...
  if [ -d "$DATASET_DIR" ]; then
    echo "📁 数据集 '$DATASET' 输出:"
    echo "  ├── details.jsonl (详细分析结果)"
    
    # 查找该数据集的图表文件
    CHARTS=$(find "$DATASET_DIR" -name "*.png" -type f | head -5)
    if [ ! -z "$CHARTS" ]; then
      echo "  └── 图表文件:"
...
echo
echo "📊 多数据集对比图:"
MULTI_CHARTS=$(find "$OUTPUT_DIR" -name "multi_proportion_bar_resp_*.png" -type f)
if [ ! -z "$MULTI_CHARTS" ]; then
  for CHART in $MULTI_CHARTS; do
    BASENAME=$(basename "$CHART")
    echo "  - $BASENAME (按response位置)"
  done
else
  echo "  (暂无多数据集对比图)"
fi
