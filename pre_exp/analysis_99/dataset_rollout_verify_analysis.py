...
def draw_length_heatmap(length_dict, args, resp_idx, dataset_name):
    # 计算每个键的平均值，并按键升序排序
    ...
    plt.ylabel('Length')
    plt.title(f'Heatmap for Verification Length (Response {resp_idx}) - {dataset_name}')

    # 保存为 PNG 文件
    output_path = os.path.join(args.output_dir, dataset_name, f"length_heatmap_resp_{resp_idx}.png")
    plt.savefig(output_path, format='png')
    plt.close()

def draw_proportion_heatmap(proportion_dict, args, resp_idx, dataset_name):
    ...
    plt.title(f'Heatmap for Verification Proportion (Response {resp_idx}) - {dataset_name}')

    # 保存为 PNG 文件
    output_path = os.path.join(args.output_dir, dataset_name, f"proportion_heatmap_resp_{resp_idx}.png")
    plt.savefig(output_path, format='png')
    plt.close()

def draw_multi_dataset_proportion_bar(dataset_stats_by_position, args):
    """
    绘制多个数据集的比例柱状图，按response位置分别绘制
    
    Args:
        dataset_stats_by_position: 嵌套字典
            level1: response索引
            level2: 数据集名称 -> (avg_think_length, avg_proportion)
        args: 命令行参数
    """
    for resp_idx, dataset_stats in dataset_stats_by_position.items():
        ...
        ax.set_title(f'Think Length and Verification Proportion Across Datasets (Response {resp_idx})')
        ax.legend()
        ...
        # 保存为 PNG 文件
        output_path = os.path.join(args.output_dir, f"multi_proportion_bar_resp_{resp_idx}.png")
        plt.tight_layout()
        plt.savefig(output_path, format='png')
        plt.close()
        print(f"Multi-dataset proportion bar chart for response {resp_idx} saved to: {output_path}")
...
def process_single_dataset(args, dataset_name, input_path):
    ...
    args.output = str(output_dir / "details.jsonl")
        ...
        for item_idx, item in tqdm(enumerate(data), total=len(data), desc=f"Processing {dataset_name}"):
            ...
            for resp_idx, resp in enumerate(responses):
                ...
                record = {
                    ...
                    "response_position": resp_idx,  # 新增字段，记录response位置
    ...
    for resp_idx, stats in position_stats.items():
        if stats['valid_samples'] > 0:  # 只绘制有有效样本的位置
            # 绘制热力图
            draw_length_heatmap(stats['length_dict'], args, resp_idx, dataset_name)
            draw_proportion_heatmap(stats['proportion_dict'], args, resp_idx, dataset_name)
            
            # 打印统计信息
            print(f"\n=== Dataset: {dataset_name}, Response Position: {resp_idx} ===")
...
def main():
    ...
    # 按response位置存储所有数据集的统计信息
    # 结构: position_stats[resp_idx][dataset_name] = (avg_think_length, avg_proportion)
    ...
    for input_path, dataset_name in zip(args.inputs, args.datasets):
        ...
        # 处理单个数据集
        dataset_position_stats = process_single_dataset(args, dataset_name, input_path)
        
        # 提取每个位置的统计信息
        for resp_idx, stats in dataset_position_stats.items():
            if stats['valid_samples'] > 0:
                ...
                position_stats[resp_idx][dataset_name] = (avg_think, avg_prop)
    
    # 绘制多个数据集的比例柱状图（按response位置）
    draw_multi_dataset_proportion_bar(position_stats, args)
    
    # 打印汇总统计
    print(f"\n{'='*50}")
    print("Summary Statistics by Response Position:")
    print('='*50)
    
    for resp_idx in sorted(position_stats.keys()):
        print(f"\nResponse Position {resp_idx}:")
        print("-" * 30)
        for dataset_name, (think_len, prop) in sorted(position_stats[resp_idx].items()):
